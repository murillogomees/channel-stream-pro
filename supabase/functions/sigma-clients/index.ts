...
async function blazeRequestWithToken(
  baseUrl: string,
  tokenType: string,
  token: string,
  endpoint: string,
  ...
  const fullUrl = joinBlazeUrl(baseUrl, endpoint);
  const resp = await blazeFetch(fullUrl, {
    method,
    headers: { ...getBlazeHeaders(fullUrl), Authorization: `${tokenType} ${token}` },
  });

  if (resp.status === 404 && endpoint.toLowerCase().startsWith("/api/")) {
    const altResp = await blazeFetch(joinBlazeUrl(baseUrl, endpoint.replace(/^\/api\//i, "/")), {
      method,
      headers: { ...getBlazeHeaders(fullUrl), Authorization: `${tokenType} ${token}` },
    });
...
serve(async (req) => {
  ...
  try {
    ...
    const hasEnvConfig =
      !!(Deno.env.get("SIGMA_BLAZE_API_URL") || "").trim() && !!(Deno.env.get("SIGMA_BLAZE_USERNAME") || "").trim();
    ...
    switch (action) {
      ...
      case "diagnostic_status": {
          ...
          try {
            ...
            for (const proxyUrl of currentProxies) {
              ...
              proxyStatsHistory.push({
                ...
                success_rate: stats && stats.total > 0 ? Math.round((stats.success / stats.total) * 100 * 10) / 10 : 0,
              });
            }
          } catch (e) {
        ...
        const overallProxySuccessRate =
          totalProxyRequests > 0 ? Math.round((totalProxySuccess / totalProxyRequests) * 100) : 0;
        ...
        if (
          supabase &&
          cacheRow?.token &&
          (cacheRow.credits === null || cacheRow.credits === undefined || !cacheRow.username)
        ) {
        ...
        if (cacheRow?.token) {
          ...
          const refreshedAtMs2 = cacheRow?.last_refreshed_at
            ? new Date(String(cacheRow.last_refreshed_at)).getTime()
            : 0;
          ...
          requests: {
            ...
            success_rate:
              totalProxyRequests > 0
                ...
                : totalRequestCount > 0
                  ? Math.round((successRequestCount / totalRequestCount) * 100)
                  : 0,
          },
          secrets,
        };
      ...
      case "get_token_status": {
        ...
        const shouldRenew = !Number.isFinite(timeUntilExpiry)
          ? true
          : timeUntilExpiry <= SESSION_CONFIG.PROACTIVE_RENEWAL_MARGIN_MS;
        ...
        if (
          effective?.token &&
          (effective.credits === null || effective.credits === undefined || !effective.username)
        ) {
      ...
      case "renew_token": {
        ...
        console.log(
          "[renew_token] Login completed, new token:",
          newToken?.slice(0, 20) + "..., expires:",
          ...
          .upsert(
            {
              id: "singleton",
              token: parsed.token,
              expires_hint: new Date(expiresAtMs).toISOString(),
      ...
      case "create_customer": {
        ...
        if (fromRenew && blazeCustomer && typeof blazeCustomer === "object") {
          ...
          if (supabase) {
            ...
            if (effectiveUserId) {
              console.log(
                `[create_customer] Triggering WhatsApp event sigma.customer_created for user: ${effectiveUserId}`,
              );
      ...
      case "probe_proxy": {
        ...
        for (const proxyUrl of proxies) {
          try {
            const client = getProxyClient(proxyUrl);
            if (!client) {
              proxyTests.push({ proxy: maskProxyUrl(proxyUrl), ok: false, error: "client_failed" });
              continue;
            }
            ...
            proxyTests.push({
              ...
              ok: false,
              error: e instanceof Error ? e.message.slice(0, 50) : "unknown",
            });
          }
        }
        result = {
          proxyConfigured: proxies.length > 0,
          proxyCount: proxies.length,
          proxyTests,
          currentWorkingProxy: currentWorkingProxy ? maskProxyUrl(currentWorkingProxy) : null,
        };
        break;
      }
      ...
      case "upsert_customer": {
        const { userId, serverId, packageId, planDurationDays, price, isTrial, username, password, blazeId, source } =
          params;
        if (!userId || !serverId || !packageId || !username || !password)
          throw new Error("upsert_customer requires: userId, serverId, packageId, username, password");
        const supabase = getSupabaseServiceClient();
        if (!supabase) throw new Error("Failed to get Supabase client");
        const { data: upsertResult, error: upsertError } = await supabase.rpc("upsert_customer_by_server", {
          p_user_id: userId,
          p_server_id: serverId,
          p_package_id: packageId,
          p_plan_duration_days: planDurationDays || 30,
          p_price: price || 0,
          p_is_trial: isTrial || false,
          p_username: username,
          p_password: password,
          p_source: source || "api",
          p_blaze_id: blazeId || null,
        });
      ...
      case "create_trial_playlist": {
        ...
        if (!userId || !serverId || !username || !password)
          throw new Error("create_trial_playlist requires: userId, serverId, username, password");
        const supabase = getSupabaseServiceClient();
        if (!supabase) throw new Error("Failed to get Supabase client");
        ...
        if (existingCustomer?.blaze_id) {
          try {
            blazeResult = await blazeRequest(`/api/customers/${existingCustomer.blaze_id}/renew`, "POST", {
              package_id: packageId || 0,
            });
          } catch {
            blazeResult = await blazeRequest(`/api/customers/${existingCustomer.blaze_id}`, "PUT", { is_trial: true });
          }
        } else {
          blazeResult = await blazeRequest("/api/customers", "POST", {
            username,
            password,
            server_id: serverId,
            package_id: packageId || 0,
            connections: 1,
            is_trial: true,
            notes: "Trial 4h - IPTV Link",
          });
        }

        const blazeId = blazeResult?.id || blazeResult?.data?.id || existingCustomer?.blaze_id;
        await supabase.rpc("upsert_customer_by_server", {
          p_user_id: userId,
          p_server_id: serverId,
          p_package_id: String(packageId || 0),
          p_plan_duration_days: 0,
          p_price: 0,
          p_is_trial: true,
          p_username: username,
          p_password: password,
          p_source: "trial",
          p_blaze_id: blazeId ? String(blazeId) : null,
        });
      ...
      case "test_panel_connection": {
        ...
        if (!base_url || !username || !password) {
          return new Response(JSON.stringify({ error: "Missing required parameters: base_url, username, password" }), {
            status: 400,
            headers: { ...corsHeaders, "Content-Type": "application/json" },
          });
        }
      ...
      case "cleanup_maxplayer": {
        ...
        const getAllCustomersCleanup = async (serverId: string) => {
          ...
          while (hasMore && page <= 200) {
            const listResult = (await blazeRequest(
              `/api/customers?page=${page}&perPage=${perPage}&server_id=${serverId}`,
              "GET",
            )) as any;
          ...
          console.log(
            `[cleanup_maxplayer] Fetched ${allCustomers.length} customers from server ${serverId} (${page - 1} pages)`,
          );
          return allCustomers;
        };
        ...
        for (const blazeClient of blazeIptvCustomers) {
          ...
          if (username) {
            ...
            if (
              !existing ||
              (blazeClient.updated_at &&
                existing.updated_at &&
                new Date(blazeClient.updated_at) > new Date(existing.updated_at))
            ) {
        ...
        console.log(
          `[cleanup_maxplayer] Found ${eligibleForCleanup.length} eligible: ${orphanCount} orphans (no Blaze), ${expiredBlazeCount} with expired Blaze`,
        );
        ...
        console.log(
          `[cleanup_maxplayer] Completed in ${durationMs}ms: deleted=${totalDeleted}, skipped=${totalSkipped}, errors=${totalErrors}`,
        );
