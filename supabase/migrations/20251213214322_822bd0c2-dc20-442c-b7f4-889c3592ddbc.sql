-- Update auto_organize to exclude movie categories from series detection
...
DECLARE
  ...
  v_parsed RECORD;
BEGIN
  -- Process channels in batches, excluding movie categories
  FOR v_channel IN 
    SELECT id, name, category 
    FROM public.iptv_channels 
    WHERE (is_series IS NOT TRUE OR series_name IS NULL)
      -- Exclude movie categories from series detection
  ...
  -- Also reset any channels in movie categories that were incorrectly marked as series
  UPDATE public.iptv_channels
  SET 
    is_series = false,
    series_name = NULL,
