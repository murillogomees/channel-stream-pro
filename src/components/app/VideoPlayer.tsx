...
export function VideoPlayer({ 
  ...
  const initPlayer = useCallback(() => {
    ...
    console.log('[VideoPlayer] Loading:', url.substring(0, 100));
    setIsLoading(true);
    setHasError(false);
    setNeedsManualPlay(false);
    cleanup();
    ...
    // Direct video (MP4, TS, Xtream live streams)
    if (!isHls) {
      ...
      video.onerror = () => {
        console.error('[VideoPlayer] Direct error');
        if (retryCount.current < 2) {
          ...
          onError?.('Direct playback failed');
        }
      };
    ...
    // HLS content
    if (Hls.isSupported()) {
      console.log('[VideoPlayer] HLS.js mode');
      ...
      // Safari native HLS
      console.log('[VideoPlayer] Native HLS');
      video.src = url;
  ...
  }, [url, cleanup, attemptPlay, onError, onReady]);
