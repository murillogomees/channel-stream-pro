...
export function GlobalSearch() {
  ...
  return <>
      <Button variant="outline" onClick={() => setOpen(true)} className="sm:pr-12 md:w-40 lg:w-64 text-left">
      ...
      <CommandDialog open={open} onOpenChange={setOpen}>
        ...
        <CommandList>
          ...
          {Object.entries(groupedItems).map(([category, items]) => <CommandGroup key={category} heading={category}>
              {items.map(item => <CommandItem key={item.path} value={`${item.title} ${item.description}`} onSelect={() => handleSelect(item.path)}>
